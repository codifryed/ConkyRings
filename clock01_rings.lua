...
show_seconds=false --true -- Change to true if you want the seconds hand
...
function conky_clock_rings()
	...
	if str=="${time %I.%M}"  then 
		strh=conky_parse("${time %I}")
		strm=conky_parse("${time %M}")
		h=tonumber(strh)
		m=tonumber(strm)
		m=m*1.67
		m=m/100
	...
	elseif str=="${time %M.%S}" then 
		strm=conky_parse("${time %M}")
		strs=conky_parse("${time %S}")
		m=tonumber(strm)
		s=tonumber(strs)
		s=s*1.67    --adjust seconds to correspond with base 100 (better drawing)
		s=s/100     --move the number down two decimal points (for graph following second hand near perfectly)
		value=m+s
	else 
		str = conky_parse(str)
		value = tonumber(str)
	...
	if value==nil then value=10 end --currently mutiple arguments in the conky_parse/tonumber function causes error.
        pct=value/pt['max']
...
function conky_top_cpu (n)
	local topproc=conky_parse(string.format('${top cpu %d}', tonumber(n)))
	local topproc_num=tonumber(topproc)
	return topproc_num*8
end
